...
void initAHCI()
{
	...
	if(ahciconfig)
	{
        ...
        pHbaMem->cap,pHbaMem->ghc,pHbaMem->is,pHbaMem->pi,pHbaMem->vs,pHbaMem->ccc_ctl
        ,pHbaMem->ccc_pts,pHbaMem->em_loc,pHbaMem->em_ctl,pHbaMem->cap2,pHbaMem->bohc);
        probe_port(abar_temp); 
        for(int i=0;i<HBA_PORT_COUNT;i++)
        {
            if(portSataDev&(((uint32_t)1)<<i))
                printf("sata dev: port %d is:0x%x ie:0x%x cmd:0x%x sig:0x%x ssts:0x%x sctl:0x%x serr:0x%x sact:0x%x\n",i,
