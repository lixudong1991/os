...
void ps2KeyInterruptProc(uint32_t code)
{
	...
	{
		if (code == 0xf0 || code == 0xe0 || code == 0xe1)
		{
	...
	else if (pkeyBoardStruct->ps2ScanIndex == 2)
	{
		uint16_t scancode = *(uint16*)(pkeyBoardStruct->ps2ScanCodeBuff);
		uint8_t subcode = pkeyBoardStruct->ps2ScanCodeBuff[2];
		if ((scancode == 0x12e0 && subcode == 0xe0) || (scancode == 0xf0e0 && subcode == 0x7c) || (scancode == 0x14e1 && subcode == 0x77))
		{
			pkeyBoardStruct->ps2ScanIndex++;
		}
		else
	...
	{
		char printscreenpressed[4] = { 0xE0, 0x12, 0xE0, 0x7C };
		char printscreenreleased[6] = { 0xE0, 0xF0, 0x7C, 0xE0, 0xF0, 0x12 };
		char pausepressed[8] = { 0xE1, 0x14, 0x77, 0xE1, 0xF0, 0x14, 0xF0, 0x77 };
		if (memcmp(printscreenpressed, pkeyBoardStruct->ps2ScanCodeBuff, 4) == 0 ||
			memcmp(printscreenreleased, pkeyBoardStruct->ps2ScanCodeBuff, 6) == 0 ||
			memcmp(pausepressed, pkeyBoardStruct->ps2ScanCodeBuff, 8) == 0 ||
			pkeyBoardStruct->ps2ScanIndex >= 8)
		{
			pkeyBoardStruct->ps2ScanIndex = 0;
		}
		else
